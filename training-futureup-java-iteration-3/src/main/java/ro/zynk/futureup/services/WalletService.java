...
//      This is what I added to buy coin P.s:I converted
        transactionRepository.save(new Transaction(LocalDate.now(),coin.getValue() * buyCoinRequest.getAmount(),(double)buyCoinRequest.getAmount(),coin));
//
    ...
    public SumResponse getSumOfAllCoinsInWallet(Long walletId) throws NotFoundException{
        Optional<Wallet> walletOpt = walletRepository.findById(walletId);
        if (walletOpt.isEmpty()) {
            throw new NotFoundException("Wallet not found!");
        }
        List<CoinAmount> coinAmounts = coinAmountRepository.findAllByWallet(walletOpt.get());
        float sumOfAllCoins = 0;
        for (CoinAmount coinAmount : coinAmounts){
            double numberOfCoins = coinAmount.getAmount();
            double valueOfThatCoin = coinAmount.getCoin().getValue();
            sumOfAllCoins += numberOfCoins * valueOfThatCoin;
        }
        return new SumResponse(sumOfAllCoins);
    }

    public ListOfTransactionsResponse getAllTransactions() {
        List<Transaction> transactionList = transactionRepository.findAll();

        return new ListOfTransactionsResponse(transactionList);
    }

    public ListOfTransactionsResponse getAllTransactionsGreaterThanPrice(Double startingPrice){
        List<Transaction> transactionList = transactionRepository.findAll();

        List<Transaction> transactionsWithMoreUSDThanStartingPrice = new ArrayList<>();

        for(Transaction transaction: transactionList)
        {
            if(transaction.getPrice() > startingPrice)
                transactionsWithMoreUSDThanStartingPrice.add(transaction);
        }
        return new ListOfTransactionsResponse(transactionsWithMoreUSDThanStartingPrice);
    }
